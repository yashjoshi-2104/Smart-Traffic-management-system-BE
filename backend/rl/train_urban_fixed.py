#!/usr/bin/env python3
"""
Urban Arterial DDQN Training Script — GPU Ready
================================================
Network : urban_arterial
TLS ID  : B1
Edges   : B2B1 (north), A1B1 (south), C1B1 (east), B0B1 (west)

SETUP (for GPU machine):
  1. Edit SUMO_BASE_PATH below to match your machine
  2. Run from DDQN_Training_Package/backend/:
       python rl/train_urban_fixed.py
"""
...
CONFIG = {
    'network':          'urban_arterial',
    'config_file':      os.path.join(SUMO_BASE_PATH, 'configs', 'urban_arterial.sumocfg'),
    'tls_id':           'B1',
    'edges': {
        'north': 'B2B1',
        'south': 'A1B1',
    ...
    'epsilon_start':    1.0,
    'epsilon_end':      0.01,
    'epsilon_decay':    0.9998,  # decays once per episode — reaches 0.01 around ep 9200
    'lr':               0.00005,
    'gamma':            0.99,
    'target_update_freq': 500,
    'save_path':        'ml/ddqn_urban.pth',
    'checkpoint_dir':   'rl/checkpoints_urban',
...
def train():
    ...
    print("=" * 80)
    print("URBAN ARTERIAL DDQN TRAINING")
    print("=" * 80)
    ...
    print(f"e decay    : {CONFIG['epsilon_decay']} (once per episode, hits 0.01 ~ep 9200)")
            ...
            for step in range(CONFIG['max_steps']):
                try:
                    ...
                    if vehicles:
                        ...
                        prev_wait    = episode_wait_times[-1] if episode_wait_times else avg_wait
                       #throughput   = sumo.get_total_vehicles_passed()
                        reward       = -avg_wait / 86.0
                    else:
                        avg_wait = 0.0
                        reward   = 0.0
    ...
    print()
    print("=" * 80)
    print("TRAINING COMPLETE - URBAN ARTERIAL")
